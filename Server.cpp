...
void Server::handleNewConnection() {
    ...
    if (tempsocket_fd < 0) {
        ...
        if (tempsocket_fd > maxfd) {
            maxfd = tempsocket_fd;
            std::cout << "incrementing maxfd to " << maxfd << std::endl;
        }
...
void Server::recvInputFromExisting(int fd) {
    std::cout << "receiving from existing...\n";
...
void Server::run() {
    int sel = 0;
    while (true) {
        std::cout << "starting server main loop\n";
        ...
        printf("select() ret %d, processing...\n", sel);
        if (sel < 0) {
